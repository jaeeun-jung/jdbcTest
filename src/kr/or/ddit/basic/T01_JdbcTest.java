...
public class T01_JdbcTest {
	...
	/*
		JDBC를 이용한 데이터베이스 처리 순서(Java DataBase Connectivity)
	 ...
	 */
		...
		try {
			// 1. 드라이버 로딩
			Class.forName("oracle.jdbc.driver.OracleDriver");			//classnotfound에러
			
			// 2. DB에 접속 (Connection객체 생성)							//로그인 에러
			...
			// SQL문이 select일 경우에는 excuteQuery()메서드 이용하고, insert, update, delete일 경우에는 executeUpdate()메서드 이용한다.
